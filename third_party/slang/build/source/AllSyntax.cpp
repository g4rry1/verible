...
void CheckerDeclarationSyntax::setChild(size_t index, TokenOrSyntax child) {
    switch (index) {
        ...
        case 5: members = child.node()->as<SyntaxList<MemberSyntax>>(); return;
        case 6: end = child.token(); return;
        case 7: endBlockName = child.node() ? &child.node()->as<NamedBlockClauseSyntax>() : nullptr; return;
        default: SLANG_UNREACHABLE;
    }
}
...
CheckerDeclarationSyntax& SyntaxFactory::checkerDeclaration(const SyntaxList<AttributeInstanceSyntax>& attributes, Token keyword, Token name, AssertionItemPortListSyntax* portList, Token semi, const SyntaxList<MemberSyntax>& members, Token end, NamedBlockClauseSyntax* endBlockName) {
    return *alloc.emplace<CheckerDeclarationSyntax>(attributes, keyword, name, portList, semi, members, end, endBlockName);
}
